...
public class AutoBlockIn extends Module {
    ...
    public AutoBlockIn() {
        super("AutoBlockIn", "AutoBlockIn", Category.PLAYER, 0, false, false);
    ...
    private double dist2PointAABB(Vec3 p, int x, int y, int z) {
        ...
        double cx = clamp(p.xCoord, x, maxX);
        double cy = clamp(p.yCoord, y, maxY);
        double cz = clamp(p.zCoord, z, maxZ);
